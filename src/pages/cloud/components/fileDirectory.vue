...
const dblclickHandle = (item) => {
  ...
  if (item.type === "picture") {
    electron.ipcRenderer.send("preview",JSON.stringify({
...
<template>
  <div id="container">
    ...
    <el-row :gutter="0" class="fileHolder" @click="changeAllChosen(false)"  v-loading="props.loading">
      <el-col
        :span="2"
        v-for="(item, index) in (filteredFiles.length === 0?props.fileTree:filteredFiles)"
        :key="index"
        :xs="8"
        :sm="6"
        :md="4"
